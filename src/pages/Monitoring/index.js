...
import PersonnelExpenditure from "./PersonnelExpenditure";
import {
  statusLabel,
} from "../ListSatuankerja/constants/styleConstants";
    ...
    <div className="bg-white dark:bg-[#111C30]/80 rounded-[20px] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.2)] border border-slate-100 dark:border-white/10 flex flex-col justify-between hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] dark:hover:border-white/20 transition-all duration-300 backdrop-blur-md">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`w-12 h-12 rounded-full flex items-center justify-center ${colorClass} ${darkColorClass}`}
          >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-2xl font-black text-slate-800 dark:text-white leading-tight mt-0.5">
              {value}
            </span>
          </div>
        </div>
...
const SummaryChart = ({ stats }) => {
  ...
  return (
    <div className="bg-white dark:bg-[#111C30]/80 backdrop-blur-md rounded-[20px] p-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 dark:border-white/10 flex flex-col h-full transition-colors">
      <h3 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-6">
          ...
          statsArray.map((item, idx) => (
            <div key={idx} className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span
                    className="text-xs font-bold text-slate-700 dark:text-slate-200 line-clamp-1"
                    title={item.title}
                  >
                    {item.title}
                  </span>
...
function MonitoringPage() {
  ...
  const [level, setLevel] = useState("um");
  const [gradeFilter, setGradeFilter] = useState("pejabat");
      ...
      if (data.success) {
        ...
        if (data.data?.last_page) {
          ...
          const calculatedPages = Math.ceil(
            Number(data.data.total) / rowsPerPage,
          );
  ...
  const fetchSppTypesThisMonth = async () => {
    try {
      ...
      if (data.success && data.data?.data) {
        ...
        const statsArray = Object.keys(grouped)
          .map((key, index) => ({
            title: key,
            value: grouped[key],
  ...
  const getStatusBadge = (status) => {
    ...
    if (s?.includes("baru"))
      return "bg-blue-50 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 border-blue-500/30";
    if (s?.includes("diproses"))
      return "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 border-emerald-500/30";
    if (s?.includes("perbaikan"))
      return "bg-amber-50 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400 border-amber-500/30";
    if (s?.includes("ditolak"))
      return "bg-red-50 text-red-600 dark:bg-red-500/20 dark:text-red-400 border-red-500/30";
    if (s?.includes("sp2d"))
      return "bg-teal-50 text-teal-600 dark:bg-teal-500/20 dark:text-teal-400 border-teal-500/30";
    ...
    <div className="w-full flex flex-col gap-5 pb-10 bg-[#f4f7fa] dark:bg-transparent min-h-screen font-sans transition-colors">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-[#111C30]/80 backdrop-blur-md rounded-[20px] p-4 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 dark:border-white/10 transition-colors">
        ...
        <div className="flex bg-slate-100 dark:bg-white/5 p-1.5 rounded-xl gap-1 w-full md:w-auto">
          {[
            { id: "um", label: "Umum" },
            ...(userData?.role !== "user" ? [{ id: "blp", label: "Belanja Pegawai" }] : []),
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => {
        ...
        {level === "blp" && (
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            >
              ...
              <option value="pegawai-pppk">Pegawai (PPPK)</option>
            </select>
          </div>
        )}
      </div>
      ...
      {level === "um" && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <StatCard
            ...
            darkColorClass="dark:bg-white/5"
            iconColor="text-slate-500 dark:text-slate-300"
            sparkline="M5 10 Q10 10 15 5 T25 12 T35 8"
          />
          <StatCard
            icon={Send}
            label="Baru"
            ...
            darkColorClass="dark:bg-blue-500/10"
            iconColor="text-blue-500 dark:text-blue-400"
            sparkline="M5 12 Q12 12 18 4 T28 10 T35 5"
          />
          <StatCard
            icon={CheckCircle2}
            label="Diproses (Lengkap)"
            ...
            darkColorClass="dark:bg-emerald-500/10"
            iconColor="text-emerald-500 dark:text-emerald-400"
            sparkline="M5 8 Q10 8 15 12 T25 4 T35 8"
          />
          <StatCard
            icon={Wrench}
            label="Butuh Perbaikan"
            ...
            darkColorClass="dark:bg-amber-500/10"
          ...
          <StatCard
            ...
            darkColorClass="dark:bg-red-500/10"
          ...
          <StatCard
            ...
            darkColorClass="dark:bg-teal-500/10"
          ...
          <div className="flex-1 w-full flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("kode_satker", e.target.value)
                }
              >
                <option value="" className="text-black dark:text-white">
                  Semua Unit Kerja (Keseluruhan)
                </option>
                {userData?.role === "user"
                  ? dataTable
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 dark:bg-[#0D1627]/50 border-b border-slate-100 dark:border-white/10">
                    <th className="px-5 py-4 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">
                      Nomor SPP
                    </th>
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                  {dataReceiptTable.length > 0 ? (
                    dataReceiptTable.map((row, index) => (
                      ...
                      >
                        <td className="px-5 py-3.5 text-xs font-bold text-slate-700 dark:text-gray-200">
                          {row.spp_number}
                        </td>
                        <td className="px-5 py-3.5 text-xs text-slate-600 dark:text-gray-300 font-medium">
                          {row.jenis_spp}
                        </td>
                        <td className="px-5 py-3.5 text-xs text-slate-500 dark:text-gray-400">
                          {row.created_at || "-"}
                        </td>
                        <td className="px-5 py-3.5 text-xs text-slate-500 dark:text-gray-400">
                          {row.time_at || "-"}
                        </td>
                        <td className="px-5 py-3.5 text-xs text-slate-600 dark:text-gray-300 font-medium capitalize">
                          {row.created_by || "-"}
                        </td>
                        <td className="px-5 py-3.5 text-center">
                          <span
                            className={`inline-flex items-center justify-center px-3 py-1 text-[7px] font-bold rounded-full border ${getStatusBadge(row.status)}`}
                          >
                    ...
                    <tr>
                      <td
                        colSpan={7}
                        className="px-5 py-16 text-sm font-medium text-slate-400 dark:text-slate-500 text-center bg-slate-50/30 dark:bg-transparent"
                      >
      ...
      {level === "blp" && <PersonnelExpenditure gradeFilter={gradeFilter}/>}
    </div>
  );
}
