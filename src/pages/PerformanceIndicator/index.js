...
function PerformanceIndicator() {
  ...
  const { dataCard } = useFetchRealization(
    "all",
    "",
    currentPeriod.month,
    currentPeriod.year,
          ...
          <div className="lg:row-span-2 relative bg-gradient-to-r from-[#59C7FF] to-[#2F8AFD] text-white rounded-xl px-7 py-16 md:py-20 lg:py-24 flex flex-col justify-between overflow-hidden h-full">
            ...
            <div className="flex flex-col gap-1 z-10">
              ...
              <span className="font-medium text-lg md:text-2xl opacity-90">
                {moment(currentPeriod?.month).format("MMM")}{" "}
                {currentPeriod?.year}
              </span>
            </div>
                ...
                <div className="flex items-center justify-start gap-2 lg:gap-4">
                  {item.value === "up" ? (
                    ...
                    <svg
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="w-12 h-12 md:w-16 md:h-16 lg:w-[30px] lg:h-[30px] text-red-500 flex-shrink-0"
                    >
                      <path d="M12 22L2 4h20z" />
                    </svg>
                  )}

                  <span className="text-xl md:text-xl lg:text-3xl font-black leading-none tracking-tight text-[#FACC15]">
                    {item.value ?? "-"}
                  </span>
                </div>
              </div>
